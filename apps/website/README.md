...
- The home-page visual content remains in `content/home.html`; Usage, Design, Plan, Security,
  ...
  time.
- `/playground/` retains the same-origin Storybook iframe and mirrors its query string into the
  canonical page URL.
...
GitHub Pages still receives static files only, and the deployment workflow continues to copy the
Storybook build into `/storybook/`. Reverting the website migration restores the former multi-entry
build; no persisted artifact or engine state requires migration. The shared cookie contains only a
supported language tag and remains valid across either website implementation.
